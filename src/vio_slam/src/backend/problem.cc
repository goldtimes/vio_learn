 ...
 * @LastEditTime: 2025-03-08 09:50:03
...
void Problem::ResizePoseHessiansWhenAddingPose(shared_ptr<Vertex> v) {
    std::cout << "H_prior_ rows:" << H_prior_.rows() << std::endl;
...
bool Problem::IsPoseVertex(std::shared_ptr<Vertex> v) {
    string type = v->TypeInfo();
    return type == string("VertexPose");
}

bool Problem::IsLandmarkVertex(std::shared_ptr<Vertex> v) {
    string type = v->TypeInfo();
...
void Problem::MakeHessian() {
    ...
    std::cout << "err_prior_ rows:" << err_prior_.rows() << std::endl;
